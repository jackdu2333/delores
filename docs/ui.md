...
- **A glyph that might be a bundled asset goes through `SymbolImage`; a fixed SF Symbol may use `Image(systemName:)`.** Some catalog symbols are bundled template assets in `Assets.xcassets` (`toggleBluetooth`), and `Image(systemName:)` silently renders nothing for those — so a name that arrives from data, a manifest or the catalog itself resolves through `SymbolImage`, which falls back to the asset of that name. A literal SF Symbol written into the view, or one that needs its own weight or rendering mode, uses `Image(systemName:)` directly; `SymbolImage` fixes both to `.regular` and `.hierarchical`.
