...
**改写后**：位移 `walkStep = 1/18`，切帧 `walkFrame = 1/9`（2:1 整倍分频），**共用一个 timer**。切帧在
`advanceWander()` 里用 `walkElapsed` 累计，每满 1/9 s 换一次姿态，两个频率精准 2:1 整倍时钟节奏不漂移。
行走驱动引入步态抓地冲量（Frame 0/2 双脚触地期权重 0.7×，Frame 1/3 单脚蹬离期权重 1.3×，循环平均恒为 1.0×），
并在 Frame 1/3 叠加入 1.5pt 踏步微颠簸（Vertical Bobbing），面板按 `backingScaleFactor` 对齐物理像素（0.5pt），
打破绝对平直平移并根除像素走样卡顿。`DeloresCompanionWander` 纯数学模型保持完全不变。
