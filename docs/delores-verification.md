...
| Show the Codex pet, then select text or drag a window onto it | The Context toolbar grows beside the Codex pet and the existing Snap Island grows beside it; a drag that starts on the Codex pet is ignored by Delores. If the pet cannot be uniquely resolved, the safe fallback above wins |
