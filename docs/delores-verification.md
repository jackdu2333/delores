...
| Select automatic mode, then show and hide Codex's pet in its settings | Delores switches live between the Codex anchor and its own pet; both pets are never active together |
| Select automatic mode when the preference key is missing or unreadable | The live DOM/WindowServer probe decides from the visible Codex pet; if it cannot resolve one, Delores uses its own pet |
| Select manual Codex mode while the Codex pet is hidden, or while its loopback overlay bridge is unavailable | The native Delores pet stays off; selection and window-snap fall back to the menu bar/top-centre affordance, with no Codex main window treated as a pet |
