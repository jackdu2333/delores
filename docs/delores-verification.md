...
| Select Codex pet mode while the Codex pet is hidden | The native Delores pet stays off; selection and window-snap fall back to the menu bar/top-centre affordance, with no Codex main window treated as a pet |
| Show the Codex pet, then select text or drag a window onto it | The Context toolbar grows beside the Codex pet and the existing Snap Island grows beside it; a drag that starts on the Codex pet is ignored by Delores. If the window is ambiguous, the safe fallback above wins |
