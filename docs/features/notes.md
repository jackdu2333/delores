...
- **The editor is the one surface text can be delivered into.** `NoteTextView` adopts
  `InjectableTextView`, so a delivered run of text is written straight into the text storage rather
  than posted as events at whichever app happens to be frontmost. Nothing else in Delores adopts it,
  and the Snippets pack that used to drive it is parked under `Packs/LegacyFeatures/`.
...
history. Delivered text expands through `insertText` and is styled like typed text.
