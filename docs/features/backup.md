...
- **AI chat history, Keychain material and anything in `Caches` never travel.** Chat history and API
  keys stay on the Mac that had them; a cache regenerates on its own.
...
exclusion carries a reason. The duplication between `AppSettings` and this file is the point: it forces
a decision about every new setting rather than defaulting it into a backup.
...
- **Settings** merge field by field and report the applied fields before returning to the pane.
